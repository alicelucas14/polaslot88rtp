...
const LiveRTPEngine = {
    ...
    init() {
        ...
        const urlProv = new URLSearchParams(window.location.search).get('game');
        const firstCard = document.querySelector(this.config.gameCardSelector);
        const cardProv = firstCard ? firstCard.getAttribute('data-prov') : '';
        const activeProvider = (urlProv || cardProv || 'default').toLowerCase();
        this.config.storageKey = 'rtpEngineGameState_v7_' + activeProvider;

        const successfulLoad = this._loadStateFromStorage();

        if (!successfulLoad) {
            console.log('Live RTP Engine: No valid saved state found for provider ' + activeProvider + '. Generating new state.');
